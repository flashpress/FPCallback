package ru.flashpress.callback
{
	...
	public class FPCaller extends EventDispatcher implements IFPTarget
	{
        ...
        //
        public static const STOP_PROPOGATION:int = 0x1;
        public static const STOP_IMMEDIATE_PROPOGATION:int = 0x2;
		//
        ...
        private var _stopImmediatePropogation:Boolean;
        ...
        private function dropParent(chanel:String, data:Object, fcaller:*, bubble:Boolean):void
        {
            ...
            _stopImmediatePropogation = false;
            if (cbList) {
                cbList._freeze = true;
                for (_i = 0; _i < cbList._count; _i++) {
                    ...
                    switch (_func.length) {
                        case 0:
                            _stopIndex = _func.call(_thisTarget);
                            break;
                        case 1:
                            _stopIndex = _func.call(_thisTarget, data);
                            break;
                        case 2:
                            _stopIndex = _func.call(_thisTarget, chanel, data);
                            break;
                        case 4:
                            _stopIndex = _func.call(_thisTarget, chanel, data, redirectTarget, fcaller);
                            break;
                    }
                    if (_stopIndex & STOP_PROPOGATION) _stopPropogation = true;
                    if (_stopIndex & STOP_IMMEDIATE_PROPOGATION) {
                        _stopImmediatePropogation = true;
                        break;
                    }
                }
        ...
        public function deleteCallbacks(chanel:String):void
        {
            _cbList6 = this.callbacksCash[chanel];
            if (_cbList6) {
                _cbList6.deleteAll();
        ...
        public function deleteAllCallbacks():void
        {
            var chanel:String;
            for (chanel in this.callbacksCash) {
                _cbList6 = this.callbacksCash[chanel];
                if(_cbList6) {
                    _cbList6.deleteAll();
                }
                delete this.callbacksCash[chanel];
            }
        }
	}
}